...
def test_and_exchange_map(tester, model, distributed):
    ...
    if distributed:
        map_tensor = torch.tensor([bbox_map, segm_map],
                                  dtype=torch.float32,
...
def train(cfg, args):
    ...
    if args.distributed:
        ...
        # else:
        # Herring: Wrap the PyTorch model with Herring’s DDP
        model = DDP(model,
    ...
    # Herring: Save model on master node.
...
def main():
    ...
    if args.distributed:
        # Herring: Pin each GPU to a single Herring process.
